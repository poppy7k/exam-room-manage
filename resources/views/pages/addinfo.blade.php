...
{{-- <form method="POST" action="{{ route('examroominfo.store') }}" style="max-width: 400px; margin: 0 auto; padding: 20px; border: 1px solid #ccc; border-radius: 5px;" onsubmit="return validateForm()">
    ...
    <div id="room-container">
        <div class="room-group" style="margin-bottom: 20px;">
            ...
            <button type="button" onclick="addSeat(this)" data-room-id="0" style="background-color: #f802cf; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;">Add Seat</button>
    ...
    <button type="button" onclick="addRoom()" style="background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;">Add Room</button>
...
<script>
    ...
    newRoom.innerHTML = `
        ...
        <button type="button" onclick="addSeat(this)" data-room-id="${roomIndex}" style="background-color: #f802cf; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;">Add Seat</button>
        <button type="button" onclick="removeRoom(this)" style="background-color: #FF0000; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;">Remove</button>
    `;
    container.appendChild(newRoom);
    roomIndex++;

    const addSeatButton = newRoom.querySelector('button[type="button"]');
    ...
    function addSeat(button) {
        const roomId = button.parentElement.getAttribute('data-room-id');
        const buildingId = "{{ $buildingId }}";
        window.location.href = `http://localhost/buildings/${buildingId}/addinfo/${roomId}/addseat`;
    }
</script> --}}
...
<form method="POST" action="{{ route('examroominfo.store') }}" style="max-width: 400px; margin: 0 auto; padding: 20px; border: 1px solid #ccc; border-radius: 5px;" onsubmit="return validateRoomForm()">
    ...
    <div id="room-container">
        <div class="room-group" data-room-id="0" style="margin-bottom: 20px;">
            <label for="floor" style="display: block; font-weight: bold; margin-bottom: 5px;">Floor</label>
            <input type="text" name="rooms[0][floor]" required style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
            <span class="error-message" style="color: red; display: none;">Floor can only be a non-negative number.</span>
            ...
            <input type="number" name="rooms[0][valid_seat]" required style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
            <span class="error-message" style="color: red; display: none;">Valid Seats can only be a non-negative number.</span>
            <button type="button" onclick="addSeat(this)" style="background-color: #f802cf; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;">Add Seat</button>
            <button type="button" onclick="removeRoom(this)" style="background-color: #FF0000; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;">Remove</button>
        </div>
    </div>
...
<script>
    ...
    function addRoom() {
        ...
        newRoom.className = 'room-group';
        newRoom.style.marginBottom = '20px';
        newRoom.setAttribute('data-room-id', roomIndex);
        newRoom.innerHTML = `
            <label for="floor" style="display: block; font-weight: bold; margin-bottom: 5px;">Floor</label>
            <input type="text" name="rooms[${roomIndex}][floor]" required style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">
            <span class="error-message" style="color: red; display: none;">Floor can only be a non-negative number.</span>
    ...
    function addSeat(button) {
        const roomId = button.closest('.room-group').getAttribute('data-room-id');
        const buildingId = "{{ $buildingId }}";
        window.location.href = `/buildings/${buildingId}/addinfo/${roomId}/addseat`;
    }
</script>
