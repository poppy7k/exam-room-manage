<div x-show="showFilterExamRoom" @click.outside="showFilterExamRoom = false"  id="filterEditRoomModal" class="absolute -translate-x-28 z-40"
        ...
        <div class="align-bottom bg-white border-2 border-gray-800/20 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="px-4 pt-2 pb-4">
                ...
                <div id="filterSort pt-2">
                    ...
                    <label onclick="window.location.href='{{ route('exam-roomlist', ['examId' => $examId, 'buildingId' => $buildingId, 'sort' => $item['value']]) }}'" for="filter-{{ $item['value'] }}" class="flex items-center w-full px-3 py-1.5 cursor-pointer transition-all duration-300 hover:bg-gray-200 rounded-md">
